...
function seedData() {
  ...
  for (let i = 29; i >= 0; i--) {
    ...
    if (Math.random() > 0.35) {
      expenses.push({
        id: uid(), date, amount: +(Math.random() * 800 + 50).toFixed(2),
  ...
  return (
    <div className={dark ? 'db-root' : 'db-root db-light'}>
      ...
      <aside className={`sidebar${sidebar ? '' : ' collapsed'}`}>
        ...
        <button className="sb-btn sb-logout" onClick={() => setShowLogoutModal(true)} title="Logout">
        ...
        <div className="modal-overlay" onClick={(e) => { if (e.target === e.currentTarget) setShowLogoutModal(false); }}>
          <div className="modal-box" style={{ maxWidth: 400, textAlign: 'center' }}>
            ...
            <p style={{ fontSize: 13, color: 'var(--text-muted)', lineHeight: 1.6, marginBottom: 10 }}>
              Are you sure you want to log out of your SmartExpense account?
            </p>
            <div style={{
              margin: '14px 0 20px',
              padding: '10px 14px',
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
              <button className="ghost-btn" onClick={() => setShowLogoutModal(false)}>
                Cancel
              </button>
